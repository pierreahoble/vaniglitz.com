...
class IndexController extends Controller
{
    ...
    public function index(){
        $user = Auth::user();
        $cars = Car::whereActive(1)->orderBy('amount')->paginate();
...
//            $reservations = Reservation::where('user_id', $user->id)->get();
    ...
    public function searchCar(Request $request){
        $data = $request->all();
        if($data['cartype_id']){
            $cars = Car::whereAvailable(1)->whereActive(1)->whereCartypeId($data['cartype_id'])->orderBy('amount')->get();
        }else{
            $cars = Car::whereAvailable(1)->whereActive(1)->orderBy('amount')->get();
        }
